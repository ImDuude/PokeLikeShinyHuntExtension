async function getState() {
  const { shinyHunt = false, targetPokemon = "" } = await chrome.storage.local.get(["shinyHunt", "targetPokemon"]);
  return { shinyHunt, targetPokemon };
}
...
async function applyStateToTab(tabId, url, state) {
  ...
  await injectFile(tabId, "pokesort_solver.js");
